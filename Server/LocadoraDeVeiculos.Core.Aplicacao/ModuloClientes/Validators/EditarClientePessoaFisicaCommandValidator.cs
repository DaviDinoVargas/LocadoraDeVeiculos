...
{
    public class EditarClientePessoaFisicaCommandValidator : AbstractValidator<EditarClientePessoaFisicaCommand>
    {
        public EditarClientePessoaFisicaCommandValidator()
        {
            RuleFor(p => p.Nome)
            ...
            RuleFor(p => p.ValidadeCnh)
                .NotEmpty().WithMessage("O campo {PropertyName} é obrigatório.")
                .GreaterThan(System.DateTime.Now).WithMessage("A CNH deve ter validade futura.");
